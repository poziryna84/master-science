...
on these samples in this lab, which means you should set a seed on top of your lab. You have to set seed every time you want to get a reproducible random result. 
...
```{r describe-sample}
mean(samp$area)#it is a "typical" size within my sample, represented by the mean of the sample
...
You know how to calculate the mean and standard deviation of 
...
<li> The sample distribution must be nearly normal. </li> True
</ol>
...
<li> 95% of random samples of size 60 will yield confidence intervals that contain the true average area of houses in Ames, Iowa. </li> True
...
The `ifelse` function takes three arguments: first is a logical statement,
...
p_value=0.99+((1-0.99)/2)
z_star_99 <- qnorm(p_value)
z_star_99
...
#Creating a variable indicates whether the interval does or does not capture the 
#true population mean.
...
#Reorganizing our data for easy plotting
