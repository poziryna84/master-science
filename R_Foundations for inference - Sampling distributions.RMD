...
```{r area-hist, eval=TRUE, fig.width=12, warning=FALSE, tidy=TRUE}
...
```{r area-stats, eval=TRUE, fig.width=12, warning=FALSE, tidy=TRUE}
ames %>%
...
```{r mean-samp1, eval=TRUE, fig.width=12, warning=FALSE, tidy=TRUE}
samp1 %>%
...
<li> Sample size of 1000. </li> this one
</ol>

Let's take one more sample of size 50, and view the mean area in this sample:
```{r mean-samp2, eval=TRUE, fig.width=12, warning=FALSE, tidy=TRUE}
ames %>%
  sample_n(size = 50) %>%
  summarise(x_bar = mean(area))
```
...
```{r loop, eval=TRUE, fig.width=12, warning=FALSE, tidy=TRUE}
sample_means50 <- ames %>%
...
```{r sampling-dist, eval=TRUE, fig.width=12, warning=FALSE, tidy=TRUE}
# type your code for the Exercise here, and Run Document

  summarise(sample_means50, s50_mean = mean(x_bar), s50_med = median(x_bar), 
            s50_stdv = sd(x_bar), s50_iqr = IQR(x_bar),
            s50_min = min(x_bar), s50_max = max(x_bar),
            s50_q1 = quantile( x_bar, 0.25),  # first quartile, 25th percentile
            s50_q3 = quantile( x_bar, 0.75)) 
```
...
```{r sample-code, eval=TRUE, fig.width=12, warning=FALSE, tidy=TRUE}
ames %>%
  sample_n(size = 50) %>%
  summarise(x_bar = mean(area))
```
...
```{r practice-sampling-dist, eval=TRUE, fig.width=12, warning=FALSE, tidy=TRUE}
# type your code for the Exercise here, and Run Document
...
```{r sample-means-small, eval=TRUE, fig.width=12, warning=FALSE, tidy=TRUE}
...
<ol>
<li> Each element represents a mean square footage from a simple random sample of 10 houses. </li>correct
...
```{r hist, eval=TRUE, fig.width=12, warning=FALSE, tidy=TRUE}
ggplot(data = sample_means50, aes(x = x_bar)) +
  geom_histogram(binwidth = 20)
```
...
```{r shiny, echo=FALSE, eval=TRUE, fig.width=12, warning=FALSE, tidy=TRUE}
shinyApp(
  ui <- fluidPage(
...
```{r price-sample, eval=TRUE, fig.width=12, warning=FALSE, tidy=TRUE}
...
```{r price-sampling, eval=TRUE, fig.width=12, warning=FALSE, tidy=TRUE}
# type your code for this Exercise here, and Run Document
sample_means50 <- ames %>%
                    rep_sample_n(size = 50, reps = 5000, replace = TRUE) %>%
                    summarise(xx_bar = mean(price))

ggplot(data = sample_means50, aes(x = xx_bar)) +
...
sample_means150 <- ames %>%
                    ...
                    summarise(xxx_bar = mean(price))

ggplot(data = sample_means150, aes(x = xxx_bar)) +
  geom_histogram(binwidth = 200)
```
* * *
...
```{r price-sample-small, eval=TRUE, fig.width=12, warning=FALSE, tidy=TRUE}
# type your code for this Exercise here, and Run Document
pr_sam_small<-ames %>%
  sample_n(size = 15) %>% 
  summarise(sam_mean=mean(price))
pr_sam_small
...
```{r price-sampling-small, eval=TRUE, fig.width=12, warning=FALSE, tidy=TRUE}
# type your code for this Exercise here, and Run Document
sample_means15 <- ames %>%
                    rep_sample_n(size = 15, reps = 2000, replace = TRUE) %>%
                    summarise(xx_bar = mean(price))

ggplot(data = sample_means15, aes(x = xx_bar)) +
  geom_histogram(binwidth = 20)
```
...
sample_means150 <- ames %>%
                    ...
                    summarise(y_bar = mean(price))

ggplot(data = sample_means150, aes(x = y_bar)) +
  geom_histogram(binwidth = 20)
```

6. Which of the following is false? 
<ol>
<li> The variability of the sampling distribution with the smaller sample size (`sample_means50`) is smaller than the variability of the sampling distribution with the larger sample size (`sample_means150`). </li> False
<li> The means for the two sampling distribtuions are roughly similar. </li> 
<li> Both sampling distributions are symmetric. </li>
</ol>
```{r price-sampling-compare, eval=TRUE, fig.width=12, warning=FALSE, tidy=TRUE}
# type your code for Question 6 here, and Run Document
summarise(sample_means150, sd(y_bar))<summarise(sample_means15, sd(xx_bar))
