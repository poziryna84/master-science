...
flights <- readr::read_csv('2008.csv/2008.csv')
...
jfk<-filter(flights, DayOfWeek %in% c(6,7), Dest=="JFK",  Distance>1000, TaxiIn<15)
...
# 3) Delete jfk object to free resources 
rm(jfk)
...
summarise(flights, min_dist=min(Distance), max_dist=max(Distance))

# Remove rows that have NA ArrDelay: temp1
no_AN<-filter(flights, !is.na(ArrDelay))
...
summarise(taxi, max(abs(TaxiOut-TaxiIn)))
...
aa %>% 
  summarise(n_flights=n(), n_canc=sum(Cancelled),
            p_canc=n_canc/n_flights, avg_delay=mean(ArrDelay, na.rm=TRUE))
...
n_canc<-sum(aa$CancellationCode=="D", na.rm=TRUE)
n_canc
#Or
summarise(aa,
          n_cana=sum(CancellationCode=="C", na.rm=T))
...
flights %>%
  ...
  filter(!is.na(diff)) %>%# you can add readr::write_csv() to save what you have
  summarise(avg=mean(diff))# you can use %>%view to see what you have
...
flights %>%
  ...
  mutate(air_time_hours = AirTime / 60)# <- name_of_the_subset (to save it as new dataset)
...
# 
  ...
  filter(!is.na(ArrDelay)) %>% 
  group_by(UniqueCarrier) %>% 
  summarise(aver = mean(ArrDelay >0)) %>% 
  
  mutate(rank = rank(aver)) %>% 
  arrange(rank)
...
flights %>% 
  group_by(UniqueCarrier) %>% 
  top_n(2, ArrDelay) %>% #number of rows to return. If `x` is grouped, this is the number of rows per group. Will include more than `n` rows if there are ties.
...
foo <- flights %>% 
  ...
  mutate_at(vars(ends_with("Delay")), funs(./2)) #mutate applies function to all yje data frame
foo
...
flights %>% 
  ...
  mutate(DepTime = coalesce(DepTime, 0L))#change NA in DepTime for 0L 0-0 and L -integer

flights %>% 
  filter(is.na(DepTime)) %>% 
  mutate(DepTime = coalesce(DepTime, CRSDepTime))#change NA in DepTime for the value from the column CRSDepTime


unique(flights$CancellationCode)
foo <- flights %>% 
  mutate(CancellationCode = na_if(CancellationCode, ""))#if there is A change it for NA
...
foo <- flights %>% 
  mutate(CancellationCode = recode(CancellationCode, "A"="Carrier", "B"="Weather", "C"="National Air System", 
                                   ...
                                   .default="Others" ))#to change the code, e.g. for "A" it will be "Carrier", etc.
rm(foo)
...
flights %>% 
  head(20) %>% 
  unite("code", UniqueCarrier, TailNum, sep = "-") %>% #joins columns with - as separator
  select(code) %>% 
  separate(code, c("code1", "code2")) %>% #separates the column in two
  separate(code2, c("code3", "code4"), -3) #separates the column in three
...
flights2 %>% 
  ...
  left_join(airports, by = c("UniqueCarrier" = "iata"))
...
library(lubridate)#one of the most powerful library to work with dates and times
today()
now()
#to see what date it was yeastarday
today()-1
