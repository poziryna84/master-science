...
ggplot(nc, mapping = aes(x=habit, y=gained))+geom_boxplot()
```
...
ggplot(nc, mapping = aes(x=habit, y=weight))+geom_boxplot()
...
<li>(6.9779 , 7.2241)</li>
<li>(38.0952 , 38.5742)</li> True
</ol>
```{r weeks-ci-99}
inference(y = weeks, data = nc, conf_level=0.99, statistic = "mean", type = "ci",  
           method = "theoretical")
...
Answer: As the confidence level lowered the confidence interval narrowed down, as the precision increases, certainty decreases.
</div>
```{r weeks-ci-90}
inference(y = weeks, data = nc, conf_level=0.90, statistic = "mean", type = "ci",  
           method = "theoretical")
...
<div id="exercise">
A pair of variables: one numerical (weeks)- length of pregnancy in weeks and one categorical 
(premie) - whether the birth was classified as premature (premie) or full-term.
Research question: how does length of pregnancy depend on weather it is a premature or full-term pregnancy.
Hypothesis: H(zero): full_term_length= premature_length, H(A): full_term_length != premature_length
</div>
```{r pick-your-own}
inference(y = weeks, x = premie, data = nc, statistic = "mean", type = "ht", null = 0, 
          alternative = "twosided", method = "theoretical")
```
<div id="exercise">
p_value = < 0.0001 which is mmuch lower than alpha (0.05), therefore we reject the H(zero) and conclude that data provide enough evidence that the mean of length of premature birth is not equal to  the mean of length of mature birth. 
</div>
```{r weeks-ci-95}
inference(y = weeks,x = premie, data = nc, conf_level=0.95, statistic = "mean", type = "ci",  
           method = "theoretical")

```
<div id="exercise">
We are 95% confident that the length of full term pregnancy is on average 5.4263 to 6.5702 weeks longer than the length of premature pregnancy.
</div>
